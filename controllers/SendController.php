...
class SendController extends BaseActiveController
{
    ...
    public function actionMsg(){
        ...
        if($result){
            ...
            if($msg_type==1){
				...
				$msg="A meet up offline has been agreed,please discuss when and where.It\'s very important thar please <font color=red>write a review</font> to each other after you meet,so that other LivAwayer can use as references.Thank you for your KINDNESS.";
    ...
    public function actionList(){
		...
		Yii::$app->db->createCommand("update message set is_read=1 where is_read=0 AND from_uid=$to_id AND to_uid=".$send_id)->execute();
		$system_ids=$send_id."_to_".$to_id;
        if(!empty($is_history) && $is_history==1){
            $result=Yii::$app->db->createCommand("SELECT * FROM  message WHERE from_uid<>to_uid AND oid<$last_id and ((from_uid in($send_id,$to_id) AND to_uid in ($send_id,$to_id) AND oid not in ($not_id) AND system_ids='0') or system_ids='$system_ids') order by oid desc limit 5 ")->queryAll();
        }else{
            $result=Yii::$app->db->createCommand("SELECT * FROM  message WHERE from_uid<>to_uid AND oid>$last_id and ((from_uid in($send_id,$to_id) AND to_uid in ($send_id,$to_id) AND oid not in ($not_id) AND system_ids='0') or system_ids='$system_ids') order by oid desc limit 5")->queryAll();
        }
        $content=[];
        ...
        $meet=Yii::$app->db->createCommand("select *,(select nickname from user where oid=meet.from_uid) as nickname from meet where from_uid in(".$send_id.",$to_id) AND to_uid in(".$send_id.",".$to_id.") AND is_comment in(0,1) order by oid desc")->queryOne();
        if(!empty($meet)){
    ...
    public function actionUnread(){
        $to_uid=$this->userId;
        $result=Yii::$app->db->createCommand("SELECT * FROM  message WHERE to_uid =$to_uid GROUP BY from_uid order by oid desc")->queryAll();
        $content=[];
        ...
        if(!empty($result)){
            foreach($result as $model){
				...
				if(!empty($rows)){
                    ...
                    'from_nickname'=>$model['from_nickname'],
                    'from_headpic'=>$model['from_headpic'],
					'msg'=>$model['msg'],
					'msg_type'=>$model['msg_type'],
					'send_time'=>date("H-d,H:i:s",$model['send_time']),
                    'count'=>0
					];
				}
		...
		return ['code'=>200,'data'=>$content,"kpn"=>Yii::$app->user->identity->overplus_KNP,"total"=>$total];
    }
	...
	public function actionJianmian(){
		...
		if($result){
			$msg="A meet up offline has been agreed,please discuss when and where.It\'s very important thar please <font color=red>write a review</font> to each other after you meet,so that other LivAwayer can use as references.Thank you for your KINDNESS.";
			$this->sendSysInfo($msg,$to_id,$send_id);
			//$msg=Yii::$app->user->identity->nickname." needs your review.";
			$msg="Review is needed.";
			$this->sendSysInfo($msg,$to_id,$send_id);
		}
		return ['code'=>200];
	}
